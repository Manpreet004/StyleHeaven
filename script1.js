...
function removeCart() {
    document.querySelector("#pcart").addEventListener("click", function(dets) {
        if (dets.target.classList.contains('remove')) {
            let cut =  adcart[dets.target.dataset.index];
            // console.log(cut);
            dets.target.closest('.addcart').remove()
        
            adcart.splice(adcart.indexOf(cut),1);
            console.log(adcart);
        }
    })
}
...
// let array = [1, 2, 3, 4, 5];
// let indexToRemove = 2; // Index of the object you want to remove
// array.splice(indexToRemove, 1); // Removes 1 element at the specified index
// console.log(array);